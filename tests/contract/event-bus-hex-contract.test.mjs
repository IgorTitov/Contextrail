 ...
 * @purpose Verify that the event-bus module satisfies its hexagonal architecture contract: required folder layout, public-api.mjs surface, expected named exports, and no deep internal imports from unit tests.
 * @sidecar event-bus-hex-contract.test.mjs.header.md
...
/**
 * Hex contract test for the event-bus module.
...
const BASE = new URL('../../modules/event-bus/', import.meta.url);

describe('event-bus hex contract', () => {
  ...
  test('public-api.mjs exports expected surface', async () => {
    ...
    const expected = ['assertEventBusPort', 'createMemoryEventBus', 'createNodeEventBus'];
  ...
  test('unit tests do not deep-import module internals', async () => {
    const testFile = await readFile(
      join(process.cwd(), 'tests', 'unit', 'event-bus.test.mjs'),
      'utf-8',
    );
