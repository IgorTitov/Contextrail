 ...
 * @purpose Verify that the api-client module satisfies its hexagonal contract: correct folder layout, required public-api exports, and no deep import violations.
 * @sidecar api-client-hex-contract.test.mjs.header.md
...
/**
 * Hex contract test for the api-client module.
...
const BASE = new URL('../../modules/api-client/', import.meta.url);

describe('api-client hex contract', () => {
  test('has required hex folders: ports, adapters', () => {
    for (const folder of ['ports', 'adapters']) {
  ...
  test('public-api.mjs exports expected surface', async () => {
    ...
    const expected = ['assertApiClientPort', 'createFetchAdapter'];
  ...
  test('unit tests do not deep-import module internals', async () => {
    const testFile = await readFile(
      join(process.cwd(), 'tests', 'unit', 'api-client.test.mjs'),
      'utf-8',
    );
    const imports = testFile.match(/from\s+['"]([^'"]+)['"]/g) || [];
    for (const imp of imports) {
      if (imp.includes('modules/api-client')) {
