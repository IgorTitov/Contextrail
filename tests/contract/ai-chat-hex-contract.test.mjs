 ...
 * @purpose Verify that the ai-chat module satisfies its hexagonal contract: correct folder layout, required public-api exports, and no deep import violations.
 * @sidecar ai-chat-hex-contract.test.mjs.header.md
...
const BASE = new URL('../../modules/ai-chat/', import.meta.url);

describe('ai-chat hex contract', () => {
  ...
  test('unit tests do not deep-import module internals', async () => {
    const testFile = await readFile(
      join(process.cwd(), 'tests', 'unit', 'ai-chat.test.mjs'),
      'utf-8',
    );
    const imports = testFile.match(/from\s+['"]([^'"]+)['"]/g) || [];
    for (const imp of imports) {
      if (imp.includes('modules/ai-chat')) {
