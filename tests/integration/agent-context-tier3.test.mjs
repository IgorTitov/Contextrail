...
function countTier3Sidecars(brief) {
  ...
  const tier3Section =
    afterTier3 === -1 ? brief.slice(tier3Start) : brief.slice(tier3Start, afterTier3);
...
describe('Tier-3 sidecar neighborhood', () => {
  ...
  it('all Tier-3 entries end with .header.md (no source content)', () => {
    ...
    const tier3Section =
      afterTier3 === -1 ? out.slice(tier3Start) : out.slice(tier3Start, afterTier3);
    ...
    for (const line of bulletLines) {
      ...
      assert.ok(path.endsWith('.header.md'), `Tier-3 entry must end with .header.md: "${path}"`);
    }
  });
  ...
  it('--neighborhood-radius=small produces fewer sidecars than medium', () => {
    const smallOut = run([
      ...
      '--budget=32000',
      '--neighborhood-radius=small',
    ]);
    const mediumOut = run([
      '--files=modules/auth/domain/auth-state.mjs',
      '--budget=32000',
      '--neighborhood-radius=medium',
    ]);
    ...
    assert.ok(
      ...
      `medium (${mediumCount}) must be >= small (${smallCount})`,
    );
  });
  ...
  it('--neighborhood-radius=medium produces fewer sidecars than large', () => {
    ...
    const largeOut = run([
      '--files=modules/auth/domain/auth-state.mjs',
      '--budget=64000',
      '--neighborhood-radius=large',
    ]);
    const mediumCount = countTier3Sidecars(mediumOut);
    const largeCount = countTier3Sidecars(largeOut);
    assert.ok(
      largeCount >= mediumCount,
      `large (${largeCount}) must be >= medium (${mediumCount})`,
    );
  });
  ...
  it('budget-aware drop: synthetic tiny budget → Tier-3 dropped with [truncated] marker; Tier-1 and Tier-2 remain', () => {
    ...
    assert.ok(
      tightOut.includes('## Architectural map'),
      'Tier-1 must be present even with tight budget',
    );
    // Tier-2 must still be present
    assert.ok(
      tightOut.includes('## Module manifests'),
      'Tier-2 must be present even with tight budget',
    );
  ...
  it('Tier-2 regression: auth manifest still present', () => {
    ...
    assert.ok(
      ...
      'Tier-2 auth manifest must still appear',
    );
  });
